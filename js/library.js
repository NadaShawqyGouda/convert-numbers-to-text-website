...
function toOrdinal(number) {
    var num = (parseFloat(number, 10)).toFixed(2);
...
function toWords(number, asOrdinal) {
    ...
    var small;
    if(currency.value == 'Riyal'){
        small = 'halala';
    };
    if(currency.value == 'Egyption pound'){
        small = 'Penny';
    };
    var addWord = 'and';
    var words;
    var num = Number.parseFloat(number);
    var newWord = num.toString();
    var done = newWord.split('.');
    var num1 = parseInt(done[0], 10);
    var num2 = done[1];
    if(!Number.isInteger(num)){
        if(num2 < 10){
            var checkNum = done[1].split('');
            if(checkNum[0] == 0){
                num2 = null;
                small ='';
                addWord = '';
    ...
    words = generateWords(num1) + ' ' +  currency.value + ' ' + addWord + ' ' + generateWords(num2) +' ' + small ;
